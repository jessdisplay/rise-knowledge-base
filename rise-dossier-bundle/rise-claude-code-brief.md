# Rise Knowledge Graph — Implementation Brief for Claude Code

**How to use this file:** give the entire file to Claude Code as the task. It is
self-contained: §1–8 are the build instructions, and Appendix A is the authoritative
relationship taxonomy (v0.3, research-validated 2026-07-05) that the schema must
implement. Where this brief and Appendix A conflict, Appendix A wins.

---

## 1. Mission and product context

Rise is a universal compliance operating system. Its core is a knowledge graph
connecting legislation → compliance frameworks → organisational policies and
procedures → forms and registers → evidence → audit findings → improvement actions,
plus risks, controls, roles, and training. The NDIS (Australia) is the first
supported framework, but nothing NDIS-specific may be hardcoded: Aged Care, ISO
9001/27001, privacy, and WHS frameworks must slot in later without redesign.

Every governed object must be able to answer four questions: why does this exist,
what requires it, what does it create, and what evidence proves it. Navigation must
work in both directions with no dead ends.

**Milestone 1 goal (this task):** prove the relationship taxonomy in Appendix A works
as a real database schema — typed nodes and edges, bitemporal effective-dating,
domain/range validation, gap-detection queries, and legislative impact fan-out —
with a small, honestly labelled seed dataset and automated tests.

---

## 2. Stack (defaults — override only if the user directs otherwise)

- PostgreSQL 16 (docker-compose for local dev)
- Plain SQL migrations in numbered files with a minimal runner; no ORM required
- TypeScript (Node 20+), node-postgres for the seed script and query helpers, vitest
  for tests
- If the user provides an existing repo, adapt to its conventions instead of these.

Rationale (opinion from the design discussion, not settled fact): a relational
database with a graph-shaped schema beats a dedicated graph DB at this stage for
transactionality, reporting, and hiring; a graph or RDF layer can be added later if
traversal ergonomics demand it.
...
- **Lookup tables:** `node_types`; `edge_types` (name, inverse_name, symmetric flag,
  description); `edge_type_rules` (edge_type, allowed from_node_type, allowed
  to_node_type) — one row per permitted pairing, seeded from Appendix A §4.
- **`nodes`:** id (uuid), node_type, tenant_id (nullable uuid — NULL means shared
  reference content such as legislation and frameworks; non-NULL means one
  organisation's content), title, ref_number (source numbering stored exactly as
  published, never renumbered), attrs (jsonb), status, version fields, created/updated
  audit columns.
- **`edges`:** every property in Appendix A §3 — id, edge_type, from_node, to_node,
  valid_from, valid_to, recorded_at, recorded_by, assertion_source,
  verification_status, confidence, strength, rationale, strength_score, note — plus
  superseded_by (nullable uuid referencing the replacing edge).
- **Constraints** (database-level where practical, otherwise enforced in the seed/query
  helpers with tests):
  1. (from_node.node_type, to_node.node_type) must be permitted for the edge_type
     (trigger against `edge_type_rules`).
  2. `RELATES_TO` requires a non-empty note.
  3. `strength`, `rationale`, `strength_score` are only valid on `MAPS_TO`;
     `confidence` only when assertion_source = 'ai'.
  4. strength_score rules per Appendix A §6: `equal` → always 10, `not_related` →
     always 0 (auto-filled); 1–9 permitted only for subset/superset/intersects;
     NULL allowed (the non-lateral N/A case).
  5. Edges are immutable in content: block in-place UPDATE of semantic fields via
     trigger; provide a `supersede_edge()` helper that inserts the replacement and
     sets superseded_by on the old row, preserving history.
- **Bitemporal support:** valid_from/valid_to (real-world) plus recorded_at (system).
  Provide an `edges_as_at(date)` function or view returning the edge set effective
  at that date.

### 3.2 Seed data — honesty rules are hard requirements

The shared (tenant_id IS NULL) layer may contain **only facts verified during
research** (sources: Federal Register of Legislation, NDIS Commission, 2026-07-05):

- National Disability Insurance Scheme Act 2013 (Cth) — Act node.
- National Disability Insurance Scheme (Provider Registration and Practice Standards)
  Rules 2018, register ID F2018L00631 — `MADE_UNDER` the Act. Leave the enabling
  provision attribute as `TODO: capture from instrument` — do not invent a section
  number.
- National Disability Insurance Scheme (Quality Indicators for NDIS Practice
  Standards) Guidelines 2018, register ID F2018N00041 — notifiable instrument node
  (distinct node_type from legislative rules).
- The two 2026 amending instruments, each with an `AMENDS` edge to its principal:
  the Provider Registration and Practice Standards Amendment (Mandatory Registration
  and Other Matters) Rules 2026, and the Quality Indicators Amendment (Supported
  Independent Living) Guidelines 2026.
- Framework node "NDIS Practice Standards" `PUBLISHED_IN` the 2018 Rules, with one
  module node "Module 2A: Implementing behaviour support plans" (verified to exist
  as a Schedule of the Rules).

**Do NOT invent Practice Standards outcomes, indicator text, section numbers, or
additional module names.** If you have network access, you may fetch real structure
from legislation.gov.au and cite what you fetched in your report. Otherwise, any
structural stand-ins must carry a `[PLACEHOLDER]` title prefix and
`attrs.fixture = true`.

The tenant layer is different: create one example organisation with a handful of
policies, procedures, work instructions, forms, registers, evidence records, an
audit with findings, improvement actions, risks, controls, roles, and training —
enough that **every one of the 25 edge types is exercised at least once**. Fictional
content is fine here (it represents a customer's documents, not the law), but flag
every row with `attrs.fixture = true`.

### 3.3 Queries (SQL views or functions), each with tests

- The gap-detection set from Appendix A §7, one view/function each.
- `impact_fanout(instrument_node_id, as_at date)` — recursive CTE traversing inbound
  `PUBLISHED_IN`/`INTERPRETS` → inbound `IMPLEMENTS` → inbound `EVIDENCES`,
  returning affected artefacts with their path.
- `trace_why(node_id)` and `trace_dependents(node_id)` — typed outbound vs inbound
  traversal (the "why does this exist" / "what depends on this" pair).
- An as-at demonstration: an edge valid through 2025 and superseded in 2026 must
  appear in `edges_as_at('2025-06-30')` and not in the current view.

### 3.4 Acceptance criteria (write a test for each)

1. Inserting an edge that violates domain/range rules fails.
2. `RELATES_TO` without a note fails.
3. `MAPS_TO` with strength = equal stores strength_score = 10 automatically; a manual
   7 on an equal mapping is rejected.
4. In-place UPDATE of an edge's semantic fields is rejected; `supersede_edge()`
   works and history is preserved.
5. `impact_fanout` from a 2026 amendment node reaches the fixture tenant's affected
   policy and evidence.
6. The as-at query returns the historical edge set correctly.
7. Each gap view returns the deliberately planted gap fixtures and nothing else.
8. All 25 edge types appear in seed data and in at least one passing test.

Deliverables: docker-compose + migrations + seed script + views/functions + tests +
a README with runnable example queries.

---

## 4. Non-goals for Milestone 1

UI, authentication, real multi-tenant isolation (RLS), AI-suggested edges, document
or file storage, Neo4j/RDF layers, and any cross-framework `MAPS_TO` content beyond
one fixture pair.

---

## 5. Decisions already made — do not relitigate silently

- Direction convention: edges point from the dependent artefact toward its authority
  or subject (Appendix A §2.5).
- Type ≠ confidence: the three-dimension split (semantic type / assertion_source /
  verification_status) is settled (Appendix A §1).
- `PUBLISHED_IN` for frameworks housed in delegated legislation; `INTERPRETS` only
  for external frameworks (Appendix A §4.1) — verified against how NDIS is actually
  published.
- Expected cardinalities are gap-rule configuration, not schema constraints.
- Edges are immutable; supersede instead of update.
- `tenant_id`-nullable as the shared/tenant split is a **spike simplification** —
  leave a TODO noting real tenancy design is future work.

If you believe a decision is wrong, say so explicitly in your final report — with
your reasoning — but implement it as specified unless it is technically impossible.

---

## 6. Open questions — leave TODOs, do not invent answers

Appendix A §9 items 2–4 are deliberately unresolved (evidence at indicator vs
outcome level; whether `OPERATIONALISES` merges into `IMPLEMENTS`; whether
`USES`/`RECORDS_TO` generalise beyond document workflows). Mark affected code with
`TODO(taxonomy-9.x)` comments rather than resolving them.

---

## 7. Reporting back

End with a short report: what was built, any deviations and why, test results, and —
most valuable — anything in the taxonomy that fought the implementation. That
feedback drives taxonomy v0.4.

---

## 8. IP note (context only; no action in this milestone)

The cross-framework mapping space contains active third-party patents (Unified
Compliance's schema/mapping families; IBM patents including one on live crosswalk
updates responding to regulatory change — see Appendix A §8.1). Building this
internal prototype is not the concern; shipping or marketing mapping features
without patent counsel review would be. Nothing in this milestone is constrained.

---

## Epistemic status of this brief

- **Verified:** the instrument names, register IDs, and relationships in §3.2, and
  everything marked verified in Appendix A §9–10 (checked 2026-07-05 against the
  Federal Register of Legislation, NDIS Commission, NIST publications, and patent
  records).
- **Recommendation/opinion:** stack choice (§2), schema shapes (§3.1), milestone
  scope and acceptance criteria.
- **Deliberately unresolved:** Appendix A §9 items 2–4 (see §6).

---

# Appendix A — Rise Relationship Taxonomy v0.3 (authoritative, verbatim)
